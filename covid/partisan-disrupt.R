...
ggplot() +
  ...
  geom_text(data=disrupted, color=blue, size=2.75, vjust=2.5, family="Lato",
            aes(x=dem, y=concerned, label=percent_first(dem))) +
  # difference column
  geom_rect(data=disrupted, aes(xmin=.5, xmax=.6, ymin=-Inf, ymax=Inf), fill="grey") +
  geom_text(data=disrupted, aes(label=paste0(diff*100, "%"), y=concerned, x=.55), fontface="bold", size=3, family="Lato") +
...
ggsave("outputs/partisan-disrupt.jpg")
