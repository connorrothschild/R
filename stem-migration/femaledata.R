...
ggplot(data=data, mapping = aes(x=year,y=propfemale),group=year) +
  geom_point() +
...
ggplot(data=data) +
  geom_point(mapping = aes(x=(year),y=propfemale), colour = "Blue") +
  geom_point(mapping = aes(x=(year),y=firstyearpropfemale),colour="Red") +
  geom_line(aes(x=(year), y=propfemale), colour = "Blue") +
  geom_line(aes(x=(year), y=firstyearpropfemale), colour = "Red") +
  ggtitle("Percent Female Undergraduate Enrollment in STEM Fields") +
  xlab("Year") +
  ylab("Percent Female") +
  theme_minimal() +
  annotate("text", x = 5, y = 16.3, label = "Class of 2010, \n proportion female STEM \n at time of enrollment", size=3, fontface=2, colour="darkred") +
  annotate("text", x = 5, y = 18.6, label = "Class of 2010, \n proportion female STEM \n at time of graduation", size=3, fontface=2, colour="darkblue") +
